...
class ESN:
    ...
    def esn_transformation(self,data,warm_up=True):
        ...
        if(warm_up):
            ...
            for i in range(warm_up_iterations):
                ...
                time_step=data[0,:]
                time_step_bias=np.reshape(time_step,newshape=(variables,1))
    ...
    def autonomous_evolution(self,scaler,initial_conditions,iterations,warm_up=True,starting_month=0,
    cycle=False,normalize=False,return_esn_representation=False):
...
#                return []
    ...
    steps_esn=10000,steps_skip=4000,steps_skip_spectrum=4000,iterations=20,normalize=True):
        ...
        'r',label="mean spectrum Reservoir",
        label_confidence="90% confidence interval spectrum Reservoir",plot_mean=True,plot_median=False)
    ...
    steps_skip=4000):
        ...
        steps_esn,warm_up=True,cycle=cycle,normalize=True,starting_month=5)

        if(data_reservoir==[]):
            return []
            
        STE=data_reservoir[:,-1]
        STE_fluct=data_reservoir[:,-1]-np.mean(data_reservoir[:,-1])
    ...
    def plotting_autonumous_evolving_time_series_basic_bifurcation(self,ds,initial_conditions,steps_esn,
        ...
        plt.title(u"\u03bc = -0.3",fontsize=25,fontweight="bold")
    ...
    indexes_predictors,):
        ...
        mse_x=np.sqrt(np.mean((predictions[:,0]-y_validation[:,0])**2))
        print(mse_x)
        ...
        mse_y=np.sqrt(np.mean((predictions[:,1]-y_validation[:,1])**2))
        print(mse_y)
